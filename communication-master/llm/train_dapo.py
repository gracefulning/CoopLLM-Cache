...
MODEL_PATH = os.getenv("DAPO_MODEL_PATH", os.path.join(PROJECT_ROOT, "models", "merge7B_exbert"))
OUTPUT_ROOT = os.getenv("DAPO_OUTPUT_ROOT", os.path.join(PROJECT_ROOT, "outputs", "dapo"))
DATA_ROOT = os.getenv("DAPO_DATA_ROOT", os.path.join(PROJECT_ROOT, "data", "dapo"))
...
ANNEAL_TEMP_START = 1.0
ANNEAL_TEMP_END   = 0.85
ANNEAL_TOP_P_START = 0.95
ANNEAL_TOP_P_END   = 0.90
...
logger = logging.getLogger("dapo_train_vanilla")
STEP_COUNTER = 0
    ...
    def _precompute_enum(bs_caches_before, allowed_sets_str, opp_eval_seq, user_bs_connections):
        ...
        if NUM_BASE_STATIONS == 1:
            for a0 in actions_per_bs[0]:
                caches_tmp = [_apply_action_to_cache(bs_caches_before[0], a0)]
                ...
                if a0 != 0:
                    best_with_write[0] = max(best_with_write[0], score)
        else:
            for a0 in actions_per_bs[0]:
                cache0_after = _apply_action_to_cache(bs_caches_before[0], a0)
                for a1 in actions_per_bs[1]:
                    cache1_after = _apply_action_to_cache(bs_caches_before[1], a1)
                    caches_tmp = [cache0_after, cache1_after]
                    score = _calculate_hit_rate(caches_tmp, opp_eval_seq[0], user_bs_connections) if len(opp_eval_seq) == 1 \
                            else _weighted_avg_hit_rate(caches_tmp, opp_eval_seq, user_bs_connections, gamma=OPP_DISCOUNT_GAMMA)
                    if a0 != 0:
                        best_with_write[0] = max(best_with_write[0], score)
                    if a1 != 0:
                        best_with_write[1] = max(best_with_write[1], score)
...
class GenParamAnnealCallback(TrainerCallback):
    ...
    def bind(self, trainer, model):
        self._trainer, self._model = trainer, model; return self
    ...
    def _apply(self, state):
        ...
        # === vLLM 路径：更新 trainer.args（GRPOConfig），保证采样退火生效 ===
...
def main():
    ...
    logger.info("正在为DAPO训练添加新的LoRA适配器...")
    ...
    anneal_cb = GenParamAnnealCallback(
        ...
        strategy=ANNEAL_STRATEGY, log_every=5
    ).bind(trainer, model)
    ...
    logger.info(f"最终DAPO LoRA适配器已保存至: {os.path.abspath(final_lora_path)}")
